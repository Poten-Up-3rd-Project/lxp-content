...
class ConfirmUploadServiceMinioDotenvIT {
    ...
    static void props(DynamicPropertyRegistry r) {
        ...
        if (endpoint != null) r.add("storage.r2.endpoint", () -> endpoint);
        if (access != null) r.add("storage.r2.access-key-id", () -> access);
        if (secret != null) r.add("storage.r2.secret-access-key", () -> secret);
        if (bucket != null) r.add("storage.r2.bucket", () -> bucket);
        r.add("storage.r2.region", () -> region);
        r.add("spring.jpa.hibernate.ddl-auto", () -> "create-drop");
        r.add("storage.r2.path-style", () -> true);
    }

    @Autowired S3Client s3;
    @Autowired ConfirmUploadService confirm;
    @Autowired ResourcePort resourceWriter;
    @Autowired ResourceQueryPort resourceQuery;
